...
type WithLogging[K comparable, V any] struct {
	cacheWrappee cache.Cache[K, V]
	logger       *slog.Logger
}

func NewWithDebugLogging[K comparable, V any](cache cache.Cache[K, V], logger *slog.Logger) *WithLogging[K, V] {
	logger = logger.With("cache", fmt.Sprintf("%T", cache))
	return &WithLogging[K, V]{
		cacheWrappee: cache,
		logger:       logger,
	}
}

func (w WithLogging[K, V]) Get(key K) (V, error) {
...
func (w WithLogging[K, V]) Set(key K, value V) error {
...
func (w WithLogging[K, V]) Delete(key K) error {
